...
int main(){
    ...
    int n;
    double txa, txb, tya, tyb, t, v;
    cin >> txa >> tya >> txb >> tyb >> t >> v;
    vector<pair<double, double>> girls;
    double x,y;
    cin >> n;
    rep(i, n){
        cin >> x >> y;
        girls.push_back(make_pair(x, y));
    }

    bool res = false;
    for(auto const& e : girls){
        double dist = sqrt(pow(e.first - txa, 2) + pow(e.second - tya, 2)) + sqrt(pow(e.first - txb, 2) + pow(e.second - tyb, 2));
        if(t*v >= dist) res = true;   
    }
